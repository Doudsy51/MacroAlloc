...
7. On that region's `APPROVE` only, automatically invoke `adapt-article-french` then `generate-article-package` (French-render mode) before starting the next region; a `BLOCKED` French artifact never reopens the `APPROVE` decision or delays the next region.
